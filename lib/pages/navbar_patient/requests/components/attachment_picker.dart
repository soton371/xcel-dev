...
class PickAttachments extends StatefulWidget {
  ...
  const PickAttachments({Key? key, @required this.imagePath, this.investigationId, this.listener}) : super(key: key);
  @override
  PickAttachmentsState createState() => PickAttachmentsState();
}
typedef UploadListener = void Function(bool isSuccess);
// typedef void UploadListener(bool isSuccess);
      ...
      child: SingleChildScrollView(
        ...
        children: <Widget>[
          Container(
            ...
            child: const Center(
              child: Text('Select Your Attachment', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w400, color: Colors.white)),
            ),
          ),
                ...
                child: GridView.count(
                  ...
                  children: List.generate(images.length, (index) {
                    ...
                    if (image is ImageUploadModel) {
                      ...
                      return Card(
                        child: IconButton(
                          icon: fileNm.endsWith("Add File") ? const Icon(Icons.file_upload , size: 50,) : const Icon(Icons.camera_alt, size: 50),
                          onPressed: () {
                            fileNm.endsWith("Add File") ? pickFile(index) :
          ...
          msg == null? Container() : Text(msg??'', style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold),),

          const SizedBox(height: 10,),

          CustomButton(btnColor: primaryColor, btnText: 'Send', onTap: (){
            if(path == null){
                setState(() {
                ...
                InvestigationService().submitAttachment(path??'').then((value) {
                  debugPrint('--hi-- $value');
                  // if(value != null){
                  if(value.isNotEmpty){
                    ...
                    Map body = {
                      ...
                      "P_PHOTOLOCA" : fileName,
                      "P_MIMETYPES" : mimeType
                    };
                    InvestigationService().submitAttachmentInfo(body).then((value) {
                      debugPrint("$value");
